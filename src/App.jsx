          ...
          table: {
            ...
            body: [
              ...
              ...Array.from(new Set(dropouts.map(s => s.category))).map(cat => {
                ...
                return [cat, { text: count.toString(), alignment: 'center' }, { text: `${pct}%`, alignment: 'center' }];
              })
            ]
          },
          layout: 'borders'
  ...
  const generateWordReport = async () => {
    const tableRows = [
      new TableRow({
        children: [
          ...
          new TableCell({ children: [new Paragraph({ text: "Total Deserción", bold: true })] })
        ]
      }),
      ...Array.from(new Set(dropouts.map(s => s.category))).map(cat => {
        const count = dropouts.filter(s => s.category === cat).length;
    ...
    const doc = new Document({
      sections: [{
        properties: {},
        children: [
          new Paragraph({ text: "DASHBOARD DE CONTINUIDAD", heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER }),
          new Paragraph({ text: "Informe Ejecutivo de Retención Académica", heading: HeadingLevel.HEADING_2, alignment: AlignmentType.CENTER }),
          new Paragraph({ text: `Fecha de emisión: ${new Date().toLocaleDateString()}`, alignment: AlignmentType.CENTER }),
          new Paragraph({ text: " " }),
          new Paragraph({ text: "1. Resumen General Académico", heading: HeadingLevel.HEADING_3 }),
          new Paragraph({ text: `En el presente análisis de continuidad, partiendo de una base de ${stats.eligibleOld} estudiantes regulares, se logró la reinscripción de ${stats.reenrolled} alumnos. Esto representa una Tasa de Retención Institucional del ${stats.reenrolledPct}%. Por otro lado, la tasa de deserción se ubica en el ${stats.lostPct}%, con un total de ${stats.lost} estudiantes perdidos.` }),
          new Paragraph({ text: " " }),
          new Paragraph({ text: "2. Indicadores Clave de Rendimiento (KPIs)", heading: HeadingLevel.HEADING_3 }),
          new Paragraph({ text: `• Transición Generacional: ${stats.transiciones} alumnos promovidos exitosamente entre categorías.` }),
          new Paragraph({ text: `• Densidad Promedio: ${stats.avgDensity} alumnos por salón activo en el periodo actual.` }),
          new Paragraph({ text: `• Comportamiento de Fuga: Se perdieron ${stats.nuevosLost} alumnos de nuevo ingreso (L01) frente a ${stats.regularesLost} alumnos regulares.` }),
          new Paragraph({ text: `• Horario Crítico: El bloque con mayor índice de fuga registrado fue "${stats.topHorarioFugas}".` }),
          new Paragraph({ text: `• Tasa de Rescate (Win-back): Se contactó a ${contactedCount} alumnos, logrando recuperar a ${rescuedCount} (${winBackRate}% de efectividad).` }),
          new Paragraph({ text: " " }),
          new Paragraph({ text: "3. Matriz de Fuga por Categoría", heading: HeadingLevel.HEADING_3 }),
          new DocxTable({
            rows: tableRows,
