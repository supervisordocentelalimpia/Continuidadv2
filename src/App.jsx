...
import {
  ...
  Phone, Upload, FileText, RefreshCw, ChevronRight, Trash2, MessageCircle, 
...
const DashboardContinuidad = () => {
  ...
  const getCrmStatusColor = (status) => {
    switch (status) {
      ...
      default: return "bg-slate-100 text-slate-600 border-slate-200"; // Pendiente
    }
  };
      ...
      <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
            ...
            <input type="file" accept="application/pdf" multiple onChange={(e) => { setPdfOldFiles(Array.from(e.target.files)); }} className="block w-full text-sm" />
            <div className="text-xs text-slate-500 mt-2">{pdfOldFiles.length ? `Seleccionados: ${pdfOldFiles.length}` : ""}</div>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
            ...
            <input type="file" accept="application/pdf" multiple onChange={(e) => { setPdfNewFiles(Array.from(e.target.files)); }} className="block w-full text-sm" />
            <div className="text-xs text-slate-500 mt-2">{pdfNewFiles.length ? `Seleccionados: ${pdfNewFiles.length}` : ""}</div>
          </div>
        </div>
